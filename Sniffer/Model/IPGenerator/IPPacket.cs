...
namespace Sniffer.Model
{
    ...
    public class IPPacket
    {
        ...
        #region Constructors
        public IPPacket(IPAddress SourceIPAddress,
            IPAddress DestinationIPAddress,
            byte[] byteBuffer, byte length)
        {
            IPHeader = new List<IPHeader>();
            IPHeader[0].SourceIPAddress = SourceIPAddress;
            IPHeader[0].DestinationIPAddress = DestinationIPAddress;

            IPHeader[0].HeaderChecksum = IPHeader[0].countSum(byteBuffer);
        }
