...
namespace Sniffer.ViewModel
{
    ...
    public class MainViewModel : ViewModelBase
    {
        ...
        private UdpGeneratorViewModel udpGeneratorViewModel = new UdpGeneratorViewModel();

        //Interface Create Capture packet
        private InterfaceMonitor monitor;
        private UdpGenerators udpgenerator;
        ...
        public MainViewModel()
        {
            ...
            OpenUDPGenerator = new RelayCommand(() => OpenUdpGeneratorExecute());
            StartGenerator = new RelayCommand(() => StartGeneratorExecute());
        ...
        /// <summary>
        /// Viewmodel for currently displayed view RaisePropertyChanged true ObservableObject
        /// </summary>
        ...
        /// <summary>
        /// Решает нужно ли добавить новый пакет в контекст
        /// </summary>
        ...
        /// <summary>
        /// Returns bool which indicates wheter the packet  given protocol rule
        /// </summary>
        ...
        private bool ApplyProtocolRule(IPPacket newPacket, List<string> ProtocolList)
        {
            foreach (string protocol in ProtocolList)
            {
                ...
                else if ((protocol.Equals("IP") || protocol.Equals("Unknown"))
                    && newPacket.IPHeader[0].TransportProtocolName.Equals("IP"))
                    return true;
        ...
        private void GetPacketHexAndCharData()
        {
            ...
            if (SelectedPacket.IPHeader[0].TransportProtocolName == "UDP")
                length = SelectedPacket.UDPPacket[0].UDPHeader[0].Length;
        ...
        private void ParseFilterConditions()
        {
            ...
            string[] allowedProtocols = { "UDP", "TCP", "ICMP", "IP", "UNKNOWN",
                                          "HTTPS", "HTTP", "SSH"};

            if (filterBox.Equals("IP"))
                filterBox = "UNKNOWN";
        ...
        private IPPacket selectedPacketUdp;
        /// <summary>
        /// Packet currently selected in FilteredPacketList
        /// </summary>
        public IPPacket SelectedPacketUdp
        {
            get
            {
                return selectedPacketUdp;
            }
            set
            {
                selectedPacketUdp = value;
                GetPacketHexAndCharData();
                // There can be only one selected packet, so the list must be empty
                SelectedPacketTreeUdp.Clear();
                SelectedPacketTreeUdp.Add(selectedPacket);
                RaisePropertyChanged("SelectedPacketUdp");
                RaisePropertyChanged("HexPacketDataUdp");
                RaisePropertyChanged("CharPacketDataUdp");
            }
        }
        ...
        /// Used to bind TreeViewItems to SelectedPacket properties
        /// </summary>
        public ObservableCollection<IPPacket> SelectedPacketTreeUdp { get; private set; }

        private string hexPacketDataUdp;
        /// <summary>
        /// Selected IP packet data in hexadecimal notation
        /// </summary>
        public string HexPacketDataUdp
        {
            get
            {
                return hexPacketDataUdp;
            }
            set
            {
                hexPacketDataUdp = value;
                RaisePropertyChanged("HexPacketDataUdp");
            }
        }

        private string charPacketDataUdp;
        /// <summary>
        /// Selected IP packet data in ASCII character notation
        /// </summary>
        public string CharPacketDataUdp
        {
            get
            {
                return charPacketDataUdp;
            }
            set
            {
                charPacketDataUdp = value;
                RaisePropertyChanged("CharPacketDataUdp");
            }
        }
        ...
        public ICommand OpenUDPGenerator { get; private set; }

        private void OpenUdpGeneratorExecute()
        {
            CurrentViewModel = udpGeneratorViewModel;
        }
        ...
        public ICommand StartGenerator
        {
            get;
            private set;
        }
        private void StartGeneratorExecute()
        {
            if (udpgenerator == null)
                udpgenerator = new UdpGenerators();
            if (Datatext == null)
            {
                Datatext = "";
            }
            CharPacketDataUdp = Datatext;
            byte[] packetData = new byte[Datatext.Length];
            StringBuilder hexStringBuilder = new StringBuilder();
            for (int i = 0; i < Datatext.Length; i++)
            {
                packetData[i] = Convert.ToByte(Datatext[i]);
                hexStringBuilder.Append(packetData[i].ToString("x2") + " ");
            }
            HexPacketDataUdp = hexStringBuilder.ToString();

            Thread tr = new Thread(testingThread);
            tr.Start();
  
            //udpgenerator.sendToSocket(SelectedInterface.InterfaceAddress);
